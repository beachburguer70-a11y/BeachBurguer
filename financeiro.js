const $=id=>document.getElementById(id), TK='bb_store_token'; let token=localStorage.getItem(TK)||'', ledgerEditingId=null, ledgerCache=[]; const moeda=v=>Number(v||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}); const hoje=()=>new Intl.DateTimeFormat('en-CA',{timeZone:'America/Sao_Paulo'}).format(new Date());
...
function renderLivro(list){ledgerCache=list||[];$('livro').innerHTML=ledgerCache.map(x=>`<div class="ledger-line"><span>${esc(x.description)}<br><small class="muted">${esc(x.source||'')}</small></span><span class="ledger-actions"><strong class="${x.type}">${x.type==='saida'?'-':'+'} ${moeda(x.amount)}</strong><button class="btn dark edit-ledger-btn" type="button" onclick="editarLancamento(${Number(x.id)})">✏️ Editar</button></span></div>`).join('')||'<p class="muted">Sem lançamentos nesta data.</p>'}
...
function cancelarEdicaoLancamento(){ledgerEditingId=null;$('lancDesc').value='';$('lancValor').value='';$('lancTipo').value='saida';$('addLanc').textContent='Lançar';$('addLanc').classList.remove('green');$('addLanc').classList.add('yellow');$('cancelEditLanc')?.classList.add('hidden')}
