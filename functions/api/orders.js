...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (action === "save_addon") {
      ...
      await supabaseRequest(env,`addons?id=eq.${id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({name,price,active:body.active!==false,target_product_ids:Array.isArray(body.target_product_ids)?body.target_product_ids.map(Number).filter(Number.isInteger):[],updated_at:new Date().toISOString()})});
      return json({ok:true});
    }
