...
const defaults={id:1,prize_enabled:true,prize_discount_percent:30,wait_atafona_min:25,wait_atafona_max:35,wait_sjb_min:60,wait_sjb_max:90,wait_local_min:25,wait_local_max:30};
...
export async function onRequestPost({request,env}){try{if(!authorized(request,env))return json({ok:false,error:"Não autorizado."},401);const b=await request.json();const a=String(b.action||"");
 ...
 if(a==="set_prize_discount"){const n=Number(b.percent);if(!Number.isFinite(n)||n<0||n>100)return json({ok:false,error:"Desconto inválido. Use um valor entre 0 e 100%."},400);const percent=Math.round(n*100)/100;await supabaseRequest(env,"operational_settings?id=eq.1",{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({prize_discount_percent:percent,updated_at:new Date().toISOString()})});return json({ok:true,settings:await cfg(env)})}
